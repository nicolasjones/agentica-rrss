...
class Band(Base):
    ...
    audience_age_range = Column(String(50))  # e.g., "18-30" (Deprecating)
    audience_age_min = Column(Integer, default=18)
    audience_age_max = Column(Integer, default=35)
    audience_location = Column(String(255))  # e.g., "CABA, Argentina" (Deprecating)
    audience_country = Column(String(100))
    audience_province = Column(String(100))
    use_regional_slang = Column(Boolean, default=False)
    tone_keywords = Column(JSON)  # ["sarcastic", "energetic", "anti-corporate"]
    values_keywords = Column(JSON)  # ["authenticity", "DIY", "community"]

    def __init__(self, **kwargs):
        if 'audience_age_min' not in kwargs:
            kwargs['audience_age_min'] = 18
        if 'audience_age_max' not in kwargs:
            kwargs['audience_age_max'] = 35
        if 'use_regional_slang' not in kwargs:
            kwargs['use_regional_slang'] = False
    ...
    strategic_batches = relationship("StrategicBatch", back_populates="band", cascade="all, delete-orphan")
...
class BatchTimeframe(str, PyEnum):
    WEEKLY = "weekly"
    BIWEEKLY = "biweekly"
    MONTHLY = "monthly"
...
class EcosystemEvent(Base):
    """A real-world event that feeds the Strategist Agent (shows, launches, BTS)."""
    ...
    strategic_posts = relationship("StrategicPost", back_populates="event", cascade="all, delete-orphan")

    __table_args__ = (
        Index("idx_ecosystem_events_band_date", "band_id", "event_date"),
    )


class StrategicBatch(Base):
    """A proposed content batch generated by the Strategist Agent."""
    __tablename__ = "strategic_batches"

    id = Column(Integer, primary_key=True, autoincrement=True)
    band_id = Column(Integer, ForeignKey("bands.id"), nullable=False)
    status = Column(Enum(BatchStatus), default=BatchStatus.PROPOSED)
    timeframe = Column(Enum(BatchTimeframe), default=BatchTimeframe.WEEKLY)
    created_at = Column(DateTime(timezone=True), default=utcnow)

    band = relationship("Band", back_populates="strategic_batches")
    posts = relationship("StrategicPost", back_populates="batch", cascade="all, delete-orphan")

    __table_args__ = (
        Index("idx_strategic_batches_band_status", "band_id", "status"),
    )


class StrategicPost(Base):
    """
    Draft post within a StrategicBatch.
    V4 Refactor: Support Concept phase (nullable caption).
    """
    __tablename__ = "strategic_posts"

    id = Column(Integer, primary_key=True, autoincrement=True)
    batch_id = Column(Integer, ForeignKey("strategic_batches.id"), nullable=False)
    event_id = Column(Integer, ForeignKey("ecosystem_events.id"), nullable=True)
    parent_post_id = Column(Integer, ForeignKey("strategic_posts.id"), nullable=True)
    
    # Platform info
    platform = Column(String(50), nullable=False)
    
    # Phase 1: Ideation (Required for MAPA mode)
    concept_title = Column(String(255), nullable=True)
    narrative_goal = Column(Text, nullable=True)
    
    # Phase 2: Signal (Optional at start, required for SIGNAL mode)
    caption = Column(Text, nullable=True) # CHANGED: Now optional to support ideas first.
    hashtags = Column(JSON)
    
    scheduled_date = Column(Date)
    scheduled_time = Column(Time, nullable=True)   # V5: HH:MM scheduling
    is_approved = Column(Boolean, default=False)
    is_manual = Column(Boolean, default=False)     # V5: user-created vs AI-generated
    created_at = Column(DateTime(timezone=True), default=utcnow)

    batch = relationship("StrategicBatch", back_populates="posts")
    event = relationship("EcosystemEvent", back_populates="strategic_posts")
    # variants relationship defined at mapper level to avoid self-ref direction ambiguity
